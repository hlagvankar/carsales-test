...
def get_late_payment_data(accounts, invoices, invoice_line_items, skus, logger):
    try:
        ...
        invoice_items_with_sku = invoice_line_items.join(skus, "item_id").cache()
        ...
        # Clean Accounts DF for company address appearing on multiline
        accounts = clean_company_address(accounts).cache()
        
        # Join with accounts to get customer details
        final_df = late_payments_summary_df.join(accounts, "account_id").cache()
